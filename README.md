...
Eduflow는 “그럴듯한 데모”가 아니라, 교육자와 에이전트가 실제로 호출해도 무너지지 않게 설계한 JSON-first AI CLI입니다. Ollama 위에서 동작하고, `explain`, `quiz`, `review`를 중심으로 구조화 출력, 안전 게이트, 오개념 분류, QA harness까지 갖추고 있습니다.
...
- `PYTHONPATH=src python3 -m unittest discover -s tests`
  `28 tests OK`
- `PYTHONPATH=src python3 -m eduflow_cli.qa_harness --mode stub`
  `100/100 pass`
- `PYTHONPATH=src python3 -m eduflow_cli.qa_harness --mode live --limit 12`
  `12/12 pass`
...
```bash
cd eduflow-cli
PYTHONPATH=src python3 -m eduflow_cli.main health
```
...
```bash
cd eduflow-cli
PYTHONPATH=src python3 -m eduflow_cli.main explain --topic "피타고라스 정리" --learner-level middle --no-save
```
...
```bash
cd eduflow-cli
PYTHONPATH=src python3 -m eduflow_cli.main quiz --topic "광합성" --learner-level elementary --no-save
```
...
```bash
cd eduflow-cli
PYTHONPATH=src python3 -m eduflow_cli.main review --submission "빛은 식물이 먹는 음식이다." --learner-level high --no-save
```

## 에이전트 친화 설계

Eduflow는 사람용 CLI처럼 보이지만, 실제로는 에이전트 호출에 맞춰져 있습니다.
...
Eduflow의 차별점은 QA를 구조로 들고 간다는 점입니다.
...
```bash
cd eduflow-cli
PYTHONPATH=src python3 -m eduflow_cli.qa_harness --mode live
```

리포트는 `.eduflow/reports/`에 JSON으로 쌓입니다.
...
이 어댑터들은 모두 얇게 유지하고, 실제 판단은 Eduflow CLI 안에서 하도록 설계했습니다.
...
이 스킬들은 모두 Eduflow CLI를 얇게 감싸는 역할만 하며,
교육 로직과 안전 로직은 Eduflow 본체 안에 유지합니다.
