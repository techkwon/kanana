...
class ExplainCliTests(unittest.TestCase):
    def test_explain_command_outputs_json(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            proc = subprocess.run(
                [sys.executable, "-m", "eduflow_cli.main", "explain", "--topic", "fractions", "--learner-level", "elementary"],
                cwd=ROOT,
                env=env,
                capture_output=True,
                text=True,
    ...
    def test_stdin_learner_level_is_not_overridden_by_parser_defaults(self):
        ...
        env["EDUFLOW_USE_STUB_RUNTIME"] = "1"
        proc = subprocess.run(
            [sys.executable, "-m", "eduflow_cli.main", "explain", "--no-save"],
            cwd=ROOT,
            env=env,
            input=json.dumps({
    ...
    def test_explain_fallback_template_keeps_input_topic_when_raw_generation_is_stub_like(self):
        ...
        env["EDUFLOW_USE_STUB_RUNTIME"] = "1"
        proc = subprocess.run(
            [sys.executable, "-m", "eduflow_cli.main", "explain", "--topic", "photosynthesis", "--no-save"],
            cwd=ROOT,
            env=env,
            input=json.dumps({
                "input": {"topic": "photosynthesis"},
