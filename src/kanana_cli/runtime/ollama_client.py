...
from kanana_cli.contracts.errors import RuntimeDependencyError


class OllamaClient:
    def __init__(self, host: Optional[str] = None, timeout: Optional[float] = None):
        settings = get_settings()
        resolved_host = host or os.getenv("KANANA_OLLAMA_HOST", settings.ollama_host)
        resolved_timeout = timeout or float(
            os.getenv("KANANA_TIMEOUT_SECONDS", str(settings.request_timeout_seconds))
        )
        self.host = resolved_host.rstrip("/")
        self.timeout = resolved_timeout
        self.use_stub_runtime = os.getenv(
            "KANANA_USE_STUB_RUNTIME",
