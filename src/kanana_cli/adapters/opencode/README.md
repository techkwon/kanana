...
OpenCode should stay a thin wrapper around the `kanana` CLI. Business logic, safety policy, session logging, and prompt construction remain in `kanana` itself.
...
2. Convert it to the `kanana` request JSON contract.
...
printf '%s' "$payload" | kanana explain
```
